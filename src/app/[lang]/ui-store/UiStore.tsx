...
import { fetchUiStore } from "@/components/Utilts/Helper";
...
const UiStore = ({ parent, lang }: any) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const uiStoreData = await fetchUiStore(lang);
  ...
  const filteredItems = uiStoreSection?.data?.filter((item: any) => {
    const navItem =
      selectedTab === 0 || item?.types === navItems[selectedTab]?.title;
    const dropdown =
      selectedTab2 === 0 ||
